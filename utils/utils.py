...
def preprocess(image_path, max_size=512, mean=(0.406, 0.456, 0.485), std=(0.225, 0.224, 0.229)):
    ori_imgs = [cv2.imread(image_path + '/' + img_path, 1) for img_path in os.listdir(image_path)]
    normalized_imgs = [(img / 255 - mean) / std for img in np.array(ori_imgs)]
...
def preprocess_video(*frame_from_video):
    max_size=512
